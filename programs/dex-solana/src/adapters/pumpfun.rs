    ...
    fn after_invoke(
        ...
        if before_sa_authority_lamports > 0 {
            ...
            if authority.key() == authority_pda::ID {
                ...
                } else if diff_sa_lamports_desc > 0 {
                    transfer_sol(
                        payer.to_account_info(),
                        authority.to_account_info(),
                        diff_sa_lamports_desc,
                        None,
                    )?;
                    log_sa_lamports_info(
                        before_sa_authority_lamports,
...
pub fn buy3<'a>(
    ...
    let bonding_curve_data = swap_accounts.bonding_curve.try_borrow_data()?;
    ...
    accounts.push(AccountMeta::new_readonly(swap_accounts.global.key(), false));
    accounts.push(AccountMeta::new(swap_accounts.fee_recipient.key(), false));
    ...
    accounts.push(AccountMeta::new(swap_accounts.associated_bonding_curve.key(), false));
    ...
    accounts.push(AccountMeta::new(swap_accounts.creator_vault.key(), false));
...
pub fn compute_fees_bps(
    ...
    if fee_config.is_some() {
        ...
        let fee_config_data = fee_config.try_borrow_data()?;
        let fee_config = parse_fee_config(&fee_config_data)?;
        return Ok(calculate_fee_tier(&fee_config.fee_tiers, market_cap)?);
    }

    let global_data = global.try_borrow_data()?;
...
pub struct PumpfunBuyAccounts2<'info> {
    ...
    pub associated_bonding_curve: &'info AccountInfo<'info>,
    pub system_program: &'info AccountInfo<'info>,
    pub wsol_program: &'info AccountInfo<'info>,
    pub token_program: &'info AccountInfo<'info>,
    pub creator_vault: &'info AccountInfo<'info>,
    pub event_authority: &'info AccountInfo<'info>,
    pub global_volume_accumulator: &'info AccountInfo<'info>,
