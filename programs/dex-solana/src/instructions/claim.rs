...
use crate::utils::{
    associate_convert_token_account, create_ata_if_needed, is_token_account,
    is_token_account_initialized, log_claim_info_after, log_claim_info_before, transfer_sol,
    transfer_token,
};
use crate::*;
...
pub fn claim_handler<'a>(ctx: Context<'_, '_, 'a, 'a, ClaimAccounts<'a>>) -> Result<()> {
    ...
    // Handle SOL claim
    if destination_account.is_some()
        && is_token_account(&ctx.accounts.destination_token_account.as_ref().unwrap())
    {
