...
fn execute_wrap_unwrap_process_v3<'info>(
    ...
    temp_wsol_account: &Option<Box<InterfaceAccount<'info, TokenAccount>>>,
    token_program: Interface<'info, TokenInterface>,
    wrap_direction: bool,
    amount: u64,
) -> Result<()> {
    if wrap_direction {
        wrap_process_v3(payer, payer_wsol_account, amount, token_program)
    } else {
        let temp_wsol_account =
            temp_wsol_account.as_ref().ok_or(ErrorCode::InvalidCommissionTemporaryTokenAccount)?;
...
/// Receiver-aware wrap/unwrap initial logging
/// - destination owner: receiver owner (wrap: token account owner, unwrap: receiver pubkey)
/// - destination balance: receiver balance (wrap: WSOL amount, unwrap: lamports)
fn log_wrap_unwrap_initial_info_with_receiver<'info>(
    payer: &Signer<'info>,
    payer_wsol_account: &InterfaceAccount<'info, TokenAccount>,
    ...
    log_swap_basic_info(order_id, &source_mint, &destination_mint, &payer.key(), receiver_owner);

    let sol_balance = payer.lamports();
    let wsol_balance = payer_wsol_account.amount;

    let before_source_balance = if wrap_direction { sol_balance } else { wsol_balance };
    let before_destination_balance = get_receiver_destination_balance(receiver, wrap_direction)?;
...
fn log_wrap_unwrap_final_info<'info>(
    ...
    let source_token_change = if commission_direction {
        amount_in
    ...
    let destination_token_change = if commission_direction {
        amount_in
    } else {
        amount_in
...
/// Receiver-aware wrap/unwrap final logging
/// - destination balance: receiver balance (wrap: WSOL amount, unwrap: lamports)
fn log_wrap_unwrap_final_info_with_receiver<'info>(
    payer: &Signer<'info>,
    payer_wsol_account: &mut InterfaceAccount<'info, TokenAccount>,
    receiver: &AccountInfo<'info>,
    commission_direction: bool,
    ...
    let after_source_balance = if wrap_direction { sol_balance_after } else { wsol_balance_after };
    let after_destination_balance = get_receiver_destination_balance(receiver, wrap_direction)?;
...
/// Read destination-side balance for receiver
/// - Wrap: receiver WSOL token account amount
/// - Unwrap: receiver lamports
fn get_receiver_destination_balance<'info>(
    receiver: &AccountInfo<'info>,
    wrap_direction: bool,
) -> Result<u64> {
    if wrap_direction {
        let data = receiver.try_borrow_data()?;
        let ta = SplTokenAccount::unpack(&data)?;
        Ok(ta.amount)
    } else {
        Ok(receiver.lamports())
    }
}
...
fn validate_tob_accounts<'info>(
    ...
    if !is_charge_sol {
        let wsol_sa_acc = wsol_sa_account.as_ref().ok_or(ErrorCode::TobWsolSaRequired)?;

        require_keys_eq!(wsol_sa_acc.key(), wsol_sa::id(), ErrorCode::InvalidWsolSa);
    }

    Ok(())
}

// ==================== With Receiver Support ====================

/// Account structure for wrap/unwrap with optional specified receiver
/// - Wrap: receiver is WSOL token account (ATA)
/// - Unwrap: receiver is system account (EOA)
#[derive(Accounts)]
pub struct PlatformFeeWrapUnwrapAccountsWithReceiver<'info> {
    #[account(mut)]
    pub payer: Signer<'info>,
...
/// Transfer to receiver based on wrap/unwrap direction
/// - Wrap: transfer WSOL from payer_wsol_account to receiver (wsol ata)
/// - Unwrap: transfer SOL from payer to receiver (eoa)
/// Transfer amount:
/// - From fee (commission_direction=true): amount_in
/// - To fee (commission_direction=false): amount_in - fees
fn transfer_to_receiver<'info>(
    accounts: &PlatformFeeWrapUnwrapAccountsWithReceiver<'info>,
    amount_in: u64,
    commission_amount: u64,
    platform_fee_amount: u64,
    commission_info: &CommissionInfo,
) -> Result<()> {
    let transfer_amount = if commission_info.commission_direction {
        // From fee: transfer full amount_in
        amount_in
    } else {
        // To fee: transfer amount_in - fees
        amount_in
            .checked_sub(commission_amount)
            .ok_or(ErrorCode::CalculationError)?
            .checked_sub(platform_fee_amount)
...
/// Validate receiver for wrap/unwrap operations
/// - Wrap: receiver must be an initialized WSOL token account (ATA)
/// - Unwrap: receiver must be a system account (EOA)
fn validate_wrap_unwrap_receiver<'info>(
    receiver: &AccountInfo<'info>,
    wrap_direction: bool,
    wsol_mint: &Pubkey,
) -> Result<Pubkey> {
    if wrap_direction {
        // Wrap operation: receiver must be an initialized WSOL token account
        require!(is_token_account_initialized(receiver), ErrorCode::ReceiverMustBeTokenAccount);
        // Verify it's a WSOL token account by checking the mint
        let data = receiver.try_borrow_data()?;
        let ta = SplTokenAccount::unpack(&data)?;
        require!(ta.mint == *wsol_mint, ErrorCode::ReceiverMustBeWsolAccount);
        Ok(ta.owner)
    } else {
        // Unwrap operation: receiver must be a system account (EOA)
        require!(
...
/// Handler for wrap/unwrap with optional specified receiver
/// Fund flow:
/// - Wrap (SOL -> WSOL): payer_wsol_account transfer WSOL -> receiver (wsol ata)
/// - Unwrap (WSOL -> SOL): payer transfer SOL -> receiver (eoa)
/// Transfer amount:
/// - From fee (commission_direction=true: amount_in
/// - To fee (commission_direction=false: amount_in - fees
pub fn platform_fee_wrap_unwrap_handler_v3_with_receiver<'a>(
    ...
    // Validate receiver based on operation type and get receiver owner (destination owner)
    let receiver_owner = validate_wrap_unwrap_receiver(
    ...
    // Log initial state and get balances
    log_wrap_unwrap_initial_info_with_receiver(
        &ctx.accounts.payer,
        &ctx.accounts.payer_wsol_account,
        &ctx.accounts.receiver,
        &receiver_owner,
    ...
    // Log final state and calculate changes
    log_wrap_unwrap_final_info_with_receiver(
        &ctx.accounts.payer,
        &mut ctx.accounts.payer_wsol_account,
        &ctx.accounts.receiver,
        commission_info.commission_direction,
